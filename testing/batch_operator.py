...
class PeriodicPartition(StatefulSource):
    def __init__(self):
        self._counter = 0
    ...
    def next_batch(self):
        res = self._counter
        self._counter += 1
        self._next_awake += timedelta(seconds=0.01)

        if self._counter >= 5_000:
            raise StopIteration()
...
class PeriodicSource(PartitionedInput):
    def list_parts(self):
        return ["singleton"]
...
flow.batch("batch", max_size=5, timeout=timedelta(seconds=1))
