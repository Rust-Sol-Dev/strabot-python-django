...
s = perf_counter()
for symbolrec in SymbolRec.objects.filter(symbol_type='stock'):
    ...
    # r.set(f'df:stock:1:{symbolrec.symbol}', serialized_df_one)

    # serialized_df_daily = pickle.dumps(df_daily)
    # r.set(f'df:stock:D:{symbolrec.symbol}', serialized_df_daily)

    df_bytes = r.get(f'df:stock:1:{symbolrec.symbol}')
